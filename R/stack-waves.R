...
# `stack_waves()` verifies that several materialized waves came from one
# master realization and stacks them into a long unit-wave table.
#
# The verification is the point. A user can row-bind waves without help, but
# not reproduce the checks: two executions of one design carry identical
# `.sample_id` values, so waves of different masters stack silently, and a
# wave edited after execution keeps a correct provenance record while its rows
# no longer match it. Both are caught here and neither is visible to a hand
# written bind.
#
# What is deliberately absent is as important. No final weight, no population
# identity, no reshaping of outcomes, no covariance. The result is data for an
# inference layer, not a design.
...
#' *different* masters stack without complaint; and a wave edited after
