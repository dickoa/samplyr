...
test_that("PPS multinomial method produces valid probabilities", {
  ...
  # Expected hits should be positive (can exceed 1 for large units/n)
  expect_true(all(result$.prob > 0))

  # Weights should be positive
  expect_true(all(result$.weight > 0))

  # Weight = 1/expected_hits
  expect_equal(result$.weight, 1 / result$.prob, tolerance = 1e-10)

  # Verify expected hits formula: n * size / total_size
  total_size <- sum(frame$size)
  n <- 4
  for (i in seq_len(nrow(result))) {
    expected_hits <- n * result$size[i] / total_size
    expect_equal(result$.prob[i], expected_hits, tolerance = 1e-10)
  }
})
...
test_that("PPS multinomial with replacement can select same unit multiple times", {
  ...
  # For with-replacement methods, .prob represents expected hits (can exceed 1)
  frame <- data.frame(
    id = 1:5,
  ...
  # Large unit (id=5) should have higher expected hits
  total_size <- sum(frame$size)
  n <- 3

  for (i in seq_len(nrow(result))) {
    unit_size <- result$size[i]
    # Expected hits = n * size_i / total_size
    expected_hits <- n * unit_size / total_size
    expect_equal(result$.prob[i], expected_hits, tolerance = 1e-10)
  }

  # Verify larger units have more expected hits
  if (nrow(result) >= 2) {
    sizes <- result$size
    probs <- result$.prob
    # Larger size should mean larger expected hits
    expect_true(all(diff(probs[order(sizes)]) >= -1e-10))
  }
})
...
test_that("PPS Chromy method produces valid results", {
  frame <- data.frame(
    id = 1:8,
    size = c(10, 20, 30, 40, 50, 60, 70, 80) # Total = 360
  )
  ...
  # Expected hits should be positive
  expect_true(all(result$.prob > 0))
  ...
  # Expected hits proportional to size
  # For this test case, expected hits = 4 * size / 360
  total_size <- sum(frame$size)
  n <- 4
  for (i in seq_len(nrow(result))) {
    expected_prob <- n * result$size[i] / total_size
...
test_that("PPS Chromy with minimum replacement can select units multiple times", {
  # When expected hits > 1, Chromy uses minimum replacement
  # (a unit appears floor(expected) or ceiling(expected) times)
  frame <- data.frame(
    id = 1:4,
  ...
  # Should have 10 rows (fixed sample size)
  expect_equal(nrow(result), 10)

  # Large unit should appear multiple times
  n_large <- sum(result$id == 4)
  expect_true(n_large >= 6) # Expected hits = 7, so 6-8 is reasonable
})

test_that("PPS Chromy gives exact sample size", {
  # Chromy always gives exactly n selections
  frame <- data.frame(
    id = 1:20,
    size = sample(10:100, 20)
  )
...
test_that("Stratified PPS Chromy works correctly", {
  ...
  # Should have 6 total (3 per stratum)
  expect_equal(nrow(result), 6)

  # 3 from each stratum
  expect_equal(sum(result$stratum == "A"), 3)
  expect_equal(sum(result$stratum == "B"), 3)
})
