...
test_that("SRS gives equal probabilities", {
  ...
  # All units have equal probability: n/N = 10/100 = 0.1
  expect_true(all(result$.prob == 0.1))
...
test_that("SRS with frac gives correct probability", {
  ...
  expect_true(all(result$.prob == 0.25))
  expect_true(all(result$.weight == 4))
})

test_that("Stratified SRS gives within-stratum probabilities", {
  frame <- data.frame(
  ...
  # Stratum A: 10/100 = 0.1
  expect_true(all(result_A$.prob == 0.1))
  expect_true(all(result_A$.weight == 10))
  

  # Stratum B: 10/200 = 0.05
  expect_true(all(result_B$.prob == 0.05))
  expect_true(all(result_B$.weight == 20))
})

# ---- Systematic Sampling ----

test_that("Systematic sampling gives equal probabilities", {
  frame <- data.frame(id = 1:100)
...
test_that("Stratified systematic gives within-stratum probabilities", {
  frame <- data.frame(
...
test_that("Bernoulli sampling gives correct probabilities", {
  frame <- data.frame(id = 1:100)
  ...
  # All selected units have prob = frac
  expect_true(all(result$.prob == 0.3))
  expect_true(all(result$.weight == 1 / 0.3))
...
test_that("Stratified Bernoulli gives stratum-specific probabilities", {
  frame <- data.frame(
    stratum = rep(c("A", "B"), each = 50),
    id = 1:100
  )
  ...
  if (nrow(result_A) > 0) {
    expect_true(all(result_A$.prob == 0.2))
    expect_true(all(result_A$.weight == 5))
  }
  
  if (nrow(result_B) > 0) {
    expect_true(all(result_B$.prob == 0.5))
    expect_true(all(result_B$.weight == 2))
  }
})

# ---- Weight = 1/prob relationship ----

test_that("All equal probability methods satisfy weight = 1/prob", {
  frame <- data.frame(id = 1:100)
