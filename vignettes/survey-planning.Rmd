...
The Spencer method accounts for the relationship between the outcome variable and the selection probabilities. Pass the outcome as a bare column name. The overall selection probabilities are extracted automatically
from `.weight`:
...
Before collecting data, you can anticipate the design effect for a cluster design using the survey-planning homogeneity measure `delta` and cluster size. This `delta` is not a generic mixed-model ICC. The calculation uses `svyplan::design_effect()` directly, not a `tbl_sample` method:
...
With a `delta` of 0.05 and 10 units per cluster, the design effect is 1.45. A `delta` of 0.15 raises it to 2.35. This helps decide the number of clusters and units per cluster during the planning stage.
