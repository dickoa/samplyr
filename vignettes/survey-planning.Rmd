...
alloc_frame <- ken_enterprises |>
  summarize(
    N_h = n(),
    S_h = sd(revenue_millions),
    mean_h = mean(revenue_millions),
    .by = sector
  ) |>
...
alloc
alloc$detail[, c("stratum", "N_h", "n_h_int", "weight")]
```
...
alloc_frame_cost <- alloc_frame |>
  mutate(cost_h = c(120, 200, 150, 130, 80, 100, 90))
...
budget_alloc
budget_alloc$detail[, c("stratum", "N_h", "cost_h", "n_h_int", "weight")]
```
...
`svyplan_cluster`) work directly with `draw()`. For `n_alloc()`, the
per-stratum allocations are passed through automatically. For other objects,
the scalar total is extracted. No manual conversion is needed.
