...
The **permanent random number** (PRN) technique, introduced by Brewer et al. (1972) and developed by Ohlsson (1995) and Cotton & Hesse (1992), answers both questions with a single mechanism: attach a stable $U(0,1)$ value to each frame unit and update it between waves.

`samplyr` supports PRN-based coordination for four selection methods: `bernoulli`, `pps_poisson`, `pps_sps`, and `pps_pareto`.

## Setup

We use `nigeria_business`, a synthetic business register with 11,617 enterprises.
...
Each enterprise has an `annual_turnover` that will serve as our measure of size for PPS sampling. We target $n = 200$ enterprises per wave.
...
Perfect overlap. This is useful when the goal is to track the same units over time, for example measuring quarterly changes in turnover within a fixed panel.

## Negative Coordination: Minimum Overlap

To minimise overlap, update the PRN values between waves using Tillé's (2006, §8.2) formula:
...
births <- tibble(enterprise_id = paste0("NEW_", sprintf("%05d", seq_len(n_births))),
                 zone = gen_data(nigeria_business$zone, n_births),
                 state = gen_data(nigeria_business$state, n_births),
                 sector = gen_data(nigeria_business$sector, n_births),
                 size_class = gen_data(nigeria_business$size_class, n_births),
...
frame <- frame |>
  mutate(pik = inclusion_prob(annual_turnover, 50),
         .by = size_class)
...
wave1_str <- sampling_design() |>
  ...
  draw(n = 50, method = "pps_pareto", mos = annual_turnover, prn = u) |>
  execute(frame, seed = 1)
...
a turnover survey ($n = 200$, PPS on `annual_turnover`) and an employment survey ($n = 150$, PPS on `employees`). The surveys have different sample sizes, different measures of size, and different stratification, but they share the same frame and the same PRN column.
...
Because `annual_turnover` and `employees` are correlated (large firms have both high turnover and many employees), the overlap under positive coordination
...
This simple shift strategy works well when the surveys have similar $\pi_k$. For more complex programmes (varying burden weights, priority orderings, or dozens of surveys), NSOs use dedicated coordination systems such as the Rivière method (Rivière 1998), the Netherlands EDS (De Ree 1983; Van Huis et al. 1994), or the Swiss method (Graf & Qualité 2014). These methods track cumulative burden per unit and permute PRNs within microstrata to achieve near-optimal coordination across the full survey programme. See Tillé (2020, §8.2.7--8.2.9) for a detailed treatment.
