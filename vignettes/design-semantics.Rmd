...
For usage tutorials, see `vignette("introduction")`. For survey export workflows, see `vignette("survey-analysis")`. For sample coordination, see `vignette("sampling-coordination")`.
...
`cluster_by()` is purely structural. It does not affect inclusion probabilities directly. They are determined by `draw()` applied to
...
(Cochran 1977, ch. 11; Sarndal et al. 1992, ch. 4.3).
...
`samplyr` stores `.certainty_k` as a logical column. At export, certainty units (\(\pi_i = 1\)) are placed in a synthetic stratum (`.cert_stratum = "certainty"`), added to the strata formula alongside user-defined strata. The certainty stratum contributes zero variance (it is a census) and does not inflate degrees of freedom (Cochran 1977, ch. 11; Sarndal et al. 1992, ch. 3.5).
...
Weights are **not** adjusted. Panels structure the output but do not affect the selection mechanism or inclusion probabilities. Each panel is a representative subsample of the full sample and for per-panel inference, the analyst could just multiply `.weight` by \(k\).
...
sampling without replacement. *Journal of Applied Statistics*, 31, 305--315.
