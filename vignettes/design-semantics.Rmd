...
Use this as a topic reference while working through the tutorials. The main
reading path is `vignette("introduction")`, `vignette("three-stage-sampling")`,
then `vignette("survey-analysis")`.

## Components, scope and transitions

A design is a sequence of stages. Each stage declares optional strata and
allocation, an optional sampling-unit key, and one selection specification.
`draw()` completes the stage. `execute()` interprets it against frames.
Without allocation, scalar `n` applies per stratum. With allocation it is a
total over those strata. At later stages, both meanings apply within each
selected parent pool. For random-size methods, the target is an expectation.

| Operation | Input | Result and statistical meaning |
|---|---|---|
| `sampling_design()` and specification verbs | Column names and selection rules | An inspectable specification, with no sample drawn |
| `frame_summary(design, frame)` | Specification and registers | Resolved pool targets and chances, without random draws |
| `execute(design, frame)` | Specification and population register(s) | A realized sample with selection metadata and weights |
| `execute(partial_sample, listing)` | An unfinished design's sample and its next register | Continuation with completed-stage probabilities retained |
| `execute(new_design, previous_sample)` | A new design and an earlier realized sample | New phase with conditional selection factors compounded with prior weights |
| `execute(master, wave = t)` | A master sample with a panel schedule | Activation by conditional selection under the frozen panel quotas |
| `as_svydesign(sample)` or `as_svrepdesign(sample)` | A supported sample and collected observations | An analysis representation, subject to the inference restrictions |

The composition laws are that previews agree with execution's resolved
chances, continuation under the same frame order and uninterrupted RNG stream
agrees with one-call execution, and serialization preserves the declared
specification. Weight interpretation must survive every transformation, and
an export must preserve the required stage information or refuse the design.
Agreement of numerical targets does not turn approximate probabilities into
exact ones. Phase-wise conditional factors also need not be unconditional
marginal inclusion probabilities.

## Assumptions at a glance

For a compact selection and inference table, see `?selection-methods`. A complete three-stage fieldwork example is in
`vignette("three-stage-sampling")`.
...
| Conditional selection laws | `add_stage()` | Stage \(k\) sampling depends only on the units selected at stage \(k - 1\) and its declared inputs | Later frame construction and selection depend on prior randomization only through selected ancestors and recorded design variables |
| FPC records method-specific stage information | `execute()` | Stores a population or cluster count for WOR and `Inf` for WR or PMR. PPS WOR is converted to \(\pi_i\) at survey export | Frame size at execution is treated as the population, one of several definitions when a population changes between waves |
| Weights are inverse selection chances, unadjusted | All methods | Uses inclusion probabilities for WOR and expected hit counts for WR or PMR | Downstream adjustment is the analyst's responsibility |
...
- \(S \subseteq U\): selected population units for a WOR design.
- \(K_i\): number of realized hits for unit \(i\) in a WR or PMR design.
- \(q_i\): stage chance, equal to \(\pi_i = \Pr(i \in S)\) for WOR and
  \(E(K_i)\) for WR or PMR.
- \(\pi_{kl} = \Pr(k \in S \text{ and } l \in S)\): joint (second-order)
  inclusion probability for WOR.
- \(E(K_k K_l)\): joint expected hits for WR or PMR.
- \(w_i = 1 / q_i\): design weight per selected unit or realized occurrence.
...
`execute()` produces **design weights only**. Calibration, trimming, and nonresponse adjustment are out of scope and belong in the estimation layer (`survey`, `srvyr`, or similar).
...
**PPS methods** (`pps_brewer`, `pps_systematic`, `pps_cps`,
...
The quantity of interest is the expected number of hits \(E(n_i) = n p_i = n \, \text{mos}_i / \sum \text{mos}\). If unit \(i\) is selected \(k\) times, `samplyr` creates \(k\) rows with a `.draw` column indexing each occurrence. The weight on every output row is \(w_i = 1 / E(n_i) = 1 / (n p_i)\). For WR the draws are independent and this is the per-hit Hansen--Hurwitz estimator. No finite population correction applies (FPC = \(\infty\)).

Chromy's sequential PPS (`pps_chromy`) is classified as *probability minimum replacement* (PMR): each unit receives \(\lfloor E(n_i) \rfloor\) or \(\lceil E(n_i) \rceil\) hits. Its occurrences are not independent WR draws. At survey export, samplyr uses the Hansen--Hurwitz approximation documented below. When all \(E(n_i) < 1\), the selection itself reduces to WOR.

**Balanced and spatially balanced sampling** (`cube`, `lpm2`, `scps`):

The cube method [@devilletille2004] selects a fixed-size sample that satisfies (or nearly satisfies) the balancing equations $\sum_{i \in S} a_{ij} / \pi_i = \sum_{i \in U} a_{ij}$ for auxiliary variables $a_j$ specified via `aux`. Without `mos`, $\pi_i = n/N$ and probabilities are equal. With `mos`, $\pi_i$ is computed by `sondage::inclusion_prob()`. A `bound()` marker adds hard adjacent-integer category-count constraints.
...
**Single-stage.** Let $q_i$ be the stage selection quantity. For WOR,
$q_i = \pi_i$. For WR or PMR, $q_i = E(K_i)$, the expected hit count.
...
**Multi-stage.** Let \(q_i^{(k)}\) denote the quantity resolved within the
clusters selected at prior stages. The compound weight is:

$$
w_i = \prod_{k=1}^{K} \frac{1}{q_i^{(k)}} = \prod_{k=1}^{K} w_i^{(k)}.
$$

For an all-WOR exact design this is the inverse product of conditional
inclusion probabilities. With a WR or PMR stage, each output row is a realized
occurrence and that stage contributes an inverse expected-hit factor.
`samplyr` joins each stage's weight to its parent by the shared cluster
variables and multiplies. The per-stage weights remain in `.weight_1`,
`.weight_2`, and so on. The `.weight` column always equals their product.
...
Two situations produce a weight for units that were never in the frame. Both
are transformations of an executed sample and neither changes the draw.

### Links to a second population

`share_weights()` is the generalized weight share method
[@lavallee2007]. A sample is selected from population \(U^A\), and estimates
are wanted for a second population \(U^B\) linked to it: children through
their parents, establishments through their enterprises, persons through their
dwellings. Every target unit \(k\) in a reached cluster \(i\) receives

$$
w_i = \sum_j \frac{I(j \in S^A)}{\pi_j} \frac{L_{ji}}{L_i},
$$

the source weights carried across the links and divided by the **population**
number of links to that cluster. Assigning one weight per cluster is what makes
unit-level and cluster-level estimates of the same total agree.

Two properties of that formula decide the whole API.

The first is that \(\pi_j\) is needed only for units actually **selected**
[@lavallee2007, section 2.2.2], which is exactly what `.weight` already holds.
That is why this is a post-execute transformation rather than a sixth verb.

The second is that \(L_i\) sums over the whole of \(U^A\), **not over the
sample**. Counting a supplied link table therefore asserts that the table is a
complete population register. Underestimating this denominator inflates the
shared weight if its numerator is held fixed. Missing links can also change
the numerator or population coverage, so their overall effect has no universal
direction. The assertion has no default: either name
a column holding the population multiplicity, or state it with
`complete_links()`.
...
The target cluster is never inferred, because the three ways of setting it
produce three different weights: a bare column is the ordinary clustered
method, `NULL` makes every target unit its own cluster, and `extend_links()`
eliminates clusters by extending the links across them
[@lavallee2007, section 5.3].

Links need not be counted. `weighted_links()` replaces the 0/1 indicator with
a non-negative importance, which costs no theory as long as each target cluster
totals more than zero [@lavallee2007, section 4.5]. Counting is the case where
every link counts for one, and the two are one implementation rather than two.

### The weight contract

A shared weight is an estimation weight and is not generally the reciprocal
of a target unit's inclusion probability. Target units can have induced
inclusion probabilities through the source selection and link structure, but
the generalized weight-sharing formula does not generally produce their
inverses. The design the object carries describes selection of the **source**
population. The result records which kind of weight it holds, and every
statistical consumer states what it does about that record rather than treating
the sample as ordinary.
...
Each refusal has its own condition class and all of them are also catchable as
`samplyr_error_weight_contract`. Neither `as_tbl_sample()` nor an ordinary
dplyr reconstruction can launder an estimation weight back into a design-weight
sample.

### Where the coverage finding fires

A target cluster with no link to the source population can never be surveyed,
and the estimator understates totals by exactly its share. `share_weights()`
records that finding and does **not** warn about it, which departs on purpose
from the convention that a condition fires where it arises.

The reason is the composed case. A transformation cannot know whether its
result will stand alone or become one component of a design over several
frames, and there a cluster one frame cannot reach is the entire reason the
second frame exists. So the warning fires where the estimate is formed:
`as_svrepdesign()` reports it for a single transformation, and for a
`stack_frames()` collection it is evaluated over the union of the frames and
fires once, naming only the clusters no frame reaches.

One component's silence about a cluster counts as coverage only if that
component was describing the same target clusters. Where that cannot be
established the union is reported as not established, rather than guessed.

There are two variance routes, and each is correct for a different reason.

`as_svrepdesign()` replicates the **source** design and applies the recorded
operator inside every replicate, so the target rows are never resampled.
Sharing after replication is a different, and wrong, number.

`as_svydesign()` exports the **source-target contributions**: one row per
link, weighted by the recorded coefficient times the source unit's design
weight. The identity behind it is that the weight share total is the
Horvitz-Thompson total of a variable derived on the source units,

$$
\sum_i w_i y_i = \sum_j \frac{I(j \in S)}{\pi_j} z_j,
\qquad z_j = \sum_i \frac{L_{ji}}{L_i} y_i,
$$

and `z` depends on whatever is being analyzed, so it cannot be formed in
advance. Expanding the contributions lets `survey` form it inside each
sampling unit instead. That is exact for any link structure, with no condition
on how many source units reach a target, provided every selected source unit
has at least one contribution. A selected source unit with no link has derived
value zero but still defines a sampling unit for the variance calculation.
Because a contribution-row design cannot retain that unit without inventing a
target row, `as_svydesign()` refuses this case. `as_svrepdesign()` remains
supported because it replicates the complete source design before applying the
sharing operator.

An unequal-probability or random-size source design is refused there. Both
take their variance from a structure indexed by the rows of the source sample,
and those rows stop being the sampled units once each appears once per
contribution.

### Frames that overlap on one population

The second situation is two registers of the same population, each incomplete.
`stack_frames()` collects samples selected independently from them, and the
theory turns out to be the same one: the multiplicity estimator for overlapping
frames **is** the weight share method with the standardized link matrix, since
the number of links to a unit is the number of frames containing it
[@lohr2021, section 3.2; @mecatti2007].

What a stack records is which frames each sampled unit belongs to. What it does
**not** do is composite the weights, because which compositing factor is
appropriate depends on the estimand and on the design effects, so it is an
estimation-time choice. `theta = NULL` at export is the multiplicity
estimator, giving every frame reaching a unit an equal share. An explicit
`theta` is Hartley's constant factor [@hartley1962] and belongs to the first
frame of the stack.

A stack also does not assert that the frames together cover the target
population. That is an assumption about the registers, not something the call
can establish, which is why the verb is not named after their union. See
`vignette("survey-analysis")` for the two export routes and what each one can
carry.
...
Systematic PPS commonly produces zero pairwise inclusion probabilities. Its
matrix can be exact while a conventional unbiased sample variance remains
unavailable for pairs that can never be sampled together. Passing that matrix
through `survey::ppsmat()` is therefore not a guarantee of stable variance
estimation.
...
To use a suitable joint matrix for single-stage variance estimation, pass it via
...
`pps_chromy` is treated identically to WR at export (no FPC, no `pps` argument). The exact Sen--Yates--Grundy formula for PMR requires \(E(n_i) E(n_j) - E(n_i n_j)\) as pairwise weights [@chromy2009, eq. 5], but `survey::ppsmat()` reads \(\pi_i\) from the diagonal, which for PMR contains \(E(n_i^2)\), not \(E(n_i)\). The Hansen--Hurwitz route is an explicit approximation. It can be strongly conservative when frame order supplies implicit stratification, but it remains more stable than the tested generalized estimator based on Monte Carlo expected hits. That candidate frequently returned negative variance estimates and is not exposed as a production path. Chauvet [-@chauvet2019] gives exact pairwise results for the related randomized without-replacement case.
...
weight factor. The rules below are the partitioning rules, while activation is
...
Positive and negative coordination across survey waves is a user-level
workflow, not a package feature. `samplyr` passes PRNs to the selection method,
the user manages their persistence and update. See
`vignette("sampling-coordination")`, including the negative-coordination update
\(u_{\text{new}} = (u - \pi) \bmod 1\) [@ohlsson1995].
