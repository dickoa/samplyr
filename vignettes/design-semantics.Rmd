...
| Conditional independence across stages | `add_stage()` | Stage \(k\) sampling depends only on the units selected at stage \(k - 1\) and its declared inputs | Later frame construction and selection depend on prior randomization only through selected ancestors and recorded design variables |
...
complete population register, and getting that wrong understates every weight
whose links are only partly recorded. The assertion has no default: either name
...
A shared weight is an estimation weight. It is not \(1/\pi\) for anything,
because the target units have no inclusion probabilities of their own, and the
design the object still carries describes the selection of the **source**
population. So the result records which kind of weight it holds, and every
