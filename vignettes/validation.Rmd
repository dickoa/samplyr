...
The goal is not to benchmark speed, and it is not a proof of correctness. A run at `R = 199` can detect a design that is plainly wrong; it cannot establish that one is right. What a pass says is that the package behaved consistently with correctness on these three designs at this replication count, which is the level of assurance a smoke test provides. The scale of the Monte Carlo uncertainty is reported below so the bands can be read for what they are.
