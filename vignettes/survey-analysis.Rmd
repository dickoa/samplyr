...
Design objects store deferred column references (bare column names in
`stratify_by()` / `cluster_by()`), which are resolved against the frame when
you validate or execute the design.
...
For replicate-based variance estimation (jackknife/BRR/bootstrap), use
`as_svrepdesign()`. This converts a `tbl_sample` to `svydesign`, then to
`svyrep.design` using `survey::as.svrepdesign()`.
...
Current scope: replicate export supports single-phase non-PPS designs.
Two-phase and PPS designs should use `as_svydesign()` (linearization).

## Exact Variance with Joint Expectations

The Brewer approximation works well in practice, but if you need exact second-order quantities, `joint_expectation()` computes them for all PPS stages. For WOR stages, this produces joint inclusion probabilities (pi_kl). For WR/PMR stages (`pps_multinomial`, `pps_chromy`), it produces joint expected hits E(n_k * n_l).
...
The supported methods and their corresponding sondage dispatch:

| samplyr method     | sondage function               | Returns                |
|--------------------|--------------------------------|------------------------|
| `pps_brewer`       | `joint_inclusion_prob(design)` | pi_kl                  |
| `pps_systematic`   | `joint_inclusion_prob(design)` | pi_kl                  |
| `pps_cps`          | `joint_inclusion_prob(design)` | pi_kl                  |
| `pps_poisson`      | `joint_inclusion_prob(design)` | pi_kl                  |
| `pps_sps`          | `joint_inclusion_prob(design)` | pi_kl                  |
| `pps_pareto`       | `joint_inclusion_prob(design)` | pi_kl                  |
| `pps_multinomial`  | `joint_expected_hits(design)`  | E(n_k * n_l)           |
| `pps_chromy`       | `joint_expected_hits(design)`  | E(n_k * n_l)           |
