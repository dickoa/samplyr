...
For the statistical semantics and assumptions underlying each operation,
see `vignette("design-semantics")`. For survey export workflows, see
`vignette("survey-analysis")`. For sample coordination with permanent
random numbers, see `vignette("sampling-coordination")`.
...
sample <- execute(design, niger_eas, seed = 101)
```
...
```{r}
sample <- execute(design, niger_eas, seed = 24)
nrow(sample)
```


The result includes the original columns plus sampling metadata: `.weight` (sampling weight), `.weight_1`, `.weight_2`, etc. (per-stage weights). For with-replacement methods, `.draw_1`, `.draw_2`, etc. index each independent draw.
...
Allocation methods are defined for total sample-size allocation, so `draw(frac = ...)`
is not supported when `alloc` is set.
...
sample <- sampling_design(title = "Optimal Allocation") |>
  stratify_by(region, alloc = "optimal",
              ...
              cost = niger_eas_cost) |>
  draw(n = 300) |>
  execute(niger_eas, seed = 9876)

sample |>
  count(region)
```

**Power allocation** [@bankier1988] uses
\(n_h \propto C_h X_h^q\), where `cv` supplies \(C_h\), `importance`
supplies \(X_h\), and `power` is \(q \in [0, 1]\).

```{r}
cv_df <- data.frame(
  region = levels(niger_eas$region),
  cv = c(0.45, 0.40, 0.22, 0.20, 0.12, 0.25, 0.28, 0.32)
)

importance_df <- data.frame(
  region = levels(niger_eas$region),
  importance = c(20, 25, 60, 75, 120, 70, 65, 90)
)
...
bind_rows(
  sample_pps |> summarise(method = "PPS", mean_hh = mean(hh_count)),
...
# How many EAs with more than 500 hh
filter(niger_eas, hh_count > 500)
...
sample_certp <- sampling_design() |>
  stratify_by(region) |>
  draw(n = 50, method = "pps_systematic", mos = hh_count,
       certainty_prop = 0.05) |>
  execute(niger_eas, seed = 12345)

# How many EAs with more than 5% of the total hh by region
niger_eas |>
  mutate(mosprop = hh_count/sum(hh_count),
         .by = region) |>
  filter(mosprop >= 0.05)
...
```{r}
set.seed(1)
niger_eas$prn <- runif(nrow(niger_eas))

sample_prn <- sampling_design(title = "Samples coordination with PRN") |>
  cluster_by(ea_id) |>
  draw(n = 50, method = "pps_sps", mos = hh_count, prn = prn) |>
  execute(niger_eas, seed = 1)
sample_prn
```

Because the PRN values are stable, re-executing the same design on the same frame produces highly overlapping samples (positive coordination). PRN is supported for `bernoulli`, `pps_poisson`, `pps_sps`, and `pps_pareto`.
...
```{r}
sample_schools <- execute(twostage_design,
                          schools_frame, seed = 3)
...
```{r}
sample_schools2 <- sampling_design(title = "Stratified two-stage cluster sampling") |>
  add_stage(label = "Districts") |>
    stratify_by(region) |>
    cluster_by(district) |>
...
The `summary()` method reports weight diagnostics including Kish's effective
sample size and design effect due to unequal weighting. For the full design
effect (including clustering and stratification), use `survey::svymean()` with
`deff = TRUE` after converting with `as_svydesign()`.
