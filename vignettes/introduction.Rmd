...
Once the EAs selected, households are listed in each of them.
...
You can also use `tidyr::uncount()` for the listing step. Because tidyr
operations strip the `tbl_sample` class, pipe the result through
`as_tbl_sample()` to restore it before passing it back to `execute()`.
