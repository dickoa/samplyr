...
Place `bound()` around a categorical variable inside `aux` to require its realised sample count to be either the floor or ceiling of the expected count. Bare auxiliary variables and count bounds can be used together:
...
keeps the weights centred on `1/f` whatever the listing finds; only the
...
with a neighbour before sampling, or to cap their rate with
...
Every execution records a frame digest by default: a compact summary of the selection pools and chances the design resolved, with no unit identifiers. The `n/N` line in the printed header and the per-stage realization lines in `summary()` come from it, and `frame_summary()` returns it as tibbles. The original frame is not needed:
